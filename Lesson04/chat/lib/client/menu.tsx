...
export class Menu extends React.Component<MenuProps, MenuState> {
    ...
    render() {
        ...
        const showCreateButton =
            menuEnabled &&
            this.state.roomname !== "" &&
            this.props.completions.indexOf(this.state.roomname) === -1;
        ...
        return (
            <div>
                ...
                <input
                    ...
                    id="username"
                    onChange={e =>
                        this.changeUsername((e.target as HTMLInputElement).value)}
                />
                <div style={menuStyle}>
                    ...
                    <input
                        ...
                        onChange={e =>
                            this.changeName((e.target as HTMLInputElement).value)}
                    />
                    {showCreateButton ? (
                        <button onClick={() => this.submit(this.state.roomname)}>
                            Create room {this.state.roomname}
                        </button>
                    ) : (
                            ""
                        )}
                    {this.props.completions.map(completion => (
                        <a
                            ...
                            key={completion}
                            style={{ display: "block" }}
                            onClick={() => this.submit(completion)}
                        >
                            {completion}
                        </a>
                    ))}
