...
class App extends React.Component<Props, State> {
	...
	render() {
		...
		if (this.state.room) {
			return (
				<Room room={this.state.room} onPost={content => this.post(content)} />
			);
		}
		return (
			<Menu
	...
	private addMessage(msg: model.Message) {
		const messages = [...this.state.room.messages, msg].slice(
			Math.max(0, this.state.room.messages.length - 10)
		);
